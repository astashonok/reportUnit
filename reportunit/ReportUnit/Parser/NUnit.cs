...
namespace ReportUnit.Parser
{
    internal class NUnit : IParser
    {
        ...
        public Report Parse(string resultsFile, string env)
        {
            ...
            report.Duration = doc.Root.Attribute("duration") != null
                ? Convert.ToDouble(doc.Root.Attribute("duration").Value)
                : 0;
            ...
            report.Passed =
                doc.Root.Attribute("passed") != null
                    ...
                    : doc.Descendants("test-case").Where(x =>
...
*/
            ...
            var testSuiteTypeAssembly = doc.Descendants("test-suite")
                .Where(x => x.Attribute("result").Value.Equals("Failed") &&
                            x.Attribute("type").Value.Equals("Assembly"));
            ...
            suites.AsParallel().ToList().ForEach(ts =>
            {
                ...
                ts.Descendants("test-case").AsParallel().ToList().ForEach(tc =>
                {
                    ...
                    test.Duration = tc.Attribute("duration") != null
                        ? Convert.ToDouble(tc.Attribute("duration").Value) * 1000
                        : 0;
                    ...
                    var id =
                        tc.Descendants("property")
                            .Where(c => c.Attribute("name").Value
                                .Equals("TestScriptID", StringComparison.CurrentCultureIgnoreCase));
                    test.Id =
                        id.Count() > 0
                            ? id.ToArray()[0].Attribute("value").Value
                            : "";
                    ...
                    var parameterizedTestElement = tc
                        ...
                        .Where(x => x.Attribute("type").Value
                            .Equals("ParameterizedMethod", StringComparison.CurrentCultureIgnoreCase))
                        .FirstOrDefault();
                    ;
                    if (null != parameterizedTestElement)
                    {
                        ...
                        var desctiption = parameterizedTestElement.Descendants("property")
                            .Where(c => c.Attribute("name").Value
                                .Equals("Description", StringComparison.CurrentCultureIgnoreCase));
                        test.Description = test.Description + desctiption.ToArray()[0].Attribute("value").Value;
                        var ts_id = parameterizedTestElement.Descendants("property")
                            .Where(c => c.Attribute("name").Value
                                .Equals("TestScriptID", StringComparison.CurrentCultureIgnoreCase));
                        test.Id = ts_id.ToArray()[0].Attribute("value").Value;
                    }
                    ...
                    test.StatusMessage =
                        tc.Element("failure") != null && tc.Element("failure").Element("stack-trace") != null
                            ? tc.Element("failure").Element("stack-trace").Value.Trim()
                            : "";

                    test.StatusMessage +=
                        tc.Element("reason") != null && tc.Element("reason").Element("message") != null
                    ...
                    test.Output = tc.Element("output") != null
                    ...
                    {
                        test.KnownIssues.Add(match.ToString().Replace("[Known issue]", "").Replace("###", ""));
                    }

                    /****************/
                    /******GetPossibleIssues**********/
                    search = "\\[Possible issue\\](.*)###";
                    matches = Regex.Matches(test.Output, search);
                    foreach (var match in matches)
                    {
                        var bugId = match.ToString().Replace("[Possible issue]", "").Replace("###", "");
                        test.PossibleIssues.Add(bugId);
                    }
                    ...
                    {
                        test.ScreenShotAttachment +=
                            tc.Element("attachments")
                                .XPathSelectElement(".//attachment[description='Screenshot']/filePath") != null
                                ? "./Report/" + Path.GetFileName(tc.Element("attachments")
                                      .XPathSelectElement(".//attachment[description='Screenshot']/filePath").Value
                                      .Trim())
                                : "";
                        test.DebugAttachment +=
                            tc.Element("attachments")
                                .XPathSelectElement(".//attachment[description='Debug']/filePath") != null
                                ? "./Report/" + Path.GetFileName(tc.Element("attachments")
                                      .XPathSelectElement(".//attachment[description='Debug']/filePath").Value.Trim())
                                : "";
                        test.InfoAttachment +=
                            tc.Element("attachments")
                                .XPathSelectElement(".//attachment[description='Info']/filePath") != null
                                ? "./Report/" + Path.GetFileName(tc.Element("attachments")
                                      .XPathSelectElement(".//attachment[description='Info']/filePath").Value.Trim())
                                : "";
                    }
        ...
        {
            if (doc.Descendants("environment").Count() == 0 || doc.Descendants("environment").First() == null)
            {
                return null;
            }
            ...
            runInfo.Info.Add("Environment Name",
                "<a href ='https://wiki.sks365.com/pages/viewpage.action?spaceKey=DT&title=Hosts+for+QAs' target='_blank'>" + report.RunEnvironment +
                "</a>");
            ...
            var dur = TimeSpan.FromSeconds(report.Duration);
            Console.WriteLine("Duration:" + dur);
            runInfo.Info.Add("Duration", dur.Hours + "h " + dur.Minutes + "m " + dur.Seconds + "s");
